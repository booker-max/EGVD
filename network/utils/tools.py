...
def batch_PSNR(img, imclean, border=0, ycbcr=False, mode="bcthw"):

    if len(img.shape) == 5:
        if mode=="bcthw":
            img = rearrange(img, "b c t h w -> (b t) c h w")
            imclean = rearrange(imclean, "b c t h w -> (b t) c h w")
        else:
            img = rearrange(img, "b t c h w -> (b t) c h w")
            imclean = rearrange(imclean, "b t c h w -> (b t) c h w")
...
def batch_SSIM(img, imclean, border=0, ycbcr=False, mode="bcthw"):
